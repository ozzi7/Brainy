...
void Get_Input_Output_Identity(vector<float>& input, vector<float>& result, int delay);
...
int main()
{
    ...
    vector<vector<float>> params(population_size, vector<float>(2*total_neurons* (total_neurons-1),0.0f));
    ...
    const int nof_test_repetition = 10;
    for(int test_iter = 0; test_iter < nof_test_repetition; ++test_iter)
    {
        brains.clear();
    ...
    cout << "Average iterations needed: " << (iters_needed / nof_test_repetition) << endl;
}
void Get_Input_Output_Identity(vector<float> & input, vector<float> & result, int delay)
{
